function [E,wh,pairs] = UserEGO(info,pairs,nq)

    % calculate A
    if ~isempty(pairs)
        count_set = UserappDistribution(info,pairs,[]);
    ...
    % find the next query
    if  nq>0
        if sum(probability_obj<(1/info.nx*1e-3))==info.nx-1>0
            ...
            Z = sparse(Z);
            model = train(ones(size(Z,1),1),Z,'-s 0 -c 1 -e 1e-6 -q');
            wh = model.w';
        else
...
%             end
            ...
            show(options,info);
            % user input u: u=1 first choice chosen, u=-1 second
            if ans==1
            ...
            if exist('E_','var')
                [E__,wh_] = UserEGO(info,pairs,nq+1);
                wh = wh + wh_;
                ...
                [E_,wh,pairs] = UserEGO(info,pairs,nq+1);
