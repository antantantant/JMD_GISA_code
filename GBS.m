function [probability_obj_set,pairs] = ...
    GBS(s,d,Xf,dX,dXID,c,inq,const,alg,queryID,pairs,w,target,nq)
    nx = size(Xf,1);
...
%         if ~isempty(pairs)&&sum(probability_obj<(1/nx*1e-3))==nx-1
        if nq>100
            ...
            [probability_obj_sub_set,pairs] = GBS(s,d,Xf,dX,dXID,c,inq,const,alg,queryID,pairs,w,target,nq+1);
